...
def process_slides(input_prompt, course_id, username):
    ...
    for slide_number, slide_content in enumerate(slides):
        prompt = f"Create a prompt for an image generative model to create an image related to this content:'{slide_content}'"
        messages = [{"role": "user", "content": prompt}]
        model_name = 'azure/gpt-4o-mini'
        response = kindo_api.call_kindo_api(model=model_name, messages=messages, max_tokens=30)
